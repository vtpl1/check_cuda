...
def get_dict_or_list(in_dict1: Any) -> Dict[str, Any]:
    in_dict = in_dict1
    try:
...
def get_flatten_dict(
    in_dict1: Union[Dict[str, Any], List[Any]], in_key: Union[str, None] = None
) -> Dict[str, str]:
    ret = dict()
...
def get_flatten_values_list(
    obj: Any, in_dict: Dict[str, str], ignore_list=[]
) -> List[Any]:
    ret = []
    for k, v in in_dict.items():
        if k not in ignore_list:
